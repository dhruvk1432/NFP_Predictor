...
def load_target_data(
    ...
    # Revised target: prefer pre-built parquet cache, fallback to legacy build
    # The cached parquet is generated by Prepare_Data/build_revised_targets.py
    # and contains the exact same output as build_revised_target().
    if target_source == 'revised':
        revised_cache_path = NFP_TARGET_DIR / f"y_{target_type}_revised.parquet"

        if revised_cache_path.exists():
            # Fast path: load pre-built revised target (~3ms vs ~1.2s legacy)
            ...
            # Legacy fallback: build from 435 individual FRED snapshots
            inc_counter("data_loader.revised_target.cache_miss", 1)
            logger.info(f"Building revised {target_type.upper()} target from raw FRED snapshots (no cache)...")
...
def get_lagged_target_features(
    ...
    # Only use data from before target_month (strict PIT)
    ...
    mom = available_df['y_mom']

    # ── P3-0: Canonical lag set {1,3,6,12} + acceleration lags {1,3,6} ──
    # Lag-1 = most recent MoM print (released last month)
    if len(available_df) >= 1:
        features[f'{prefix}_mom_lag1']  = mom.iloc[-1]
        features[f'{prefix}_level_lag1'] = available_df['y'].iloc[-1]
    if len(available_df) >= 3:
        features[f'{prefix}_mom_lag3']  = mom.iloc[-3]
    if len(available_df) >= 6:
        features[f'{prefix}_mom_lag6']  = mom.iloc[-6]
    if len(available_df) >= 12:
        features[f'{prefix}_mom_lag12'] = mom.iloc[-12]

    # Acceleration lags: accel[t] = MoM[t] - MoM[t-1], shifted so always PIT-safe
    # accel_lag1 = the acceleration that was observable 1 month ago
    if len(available_df) >= 2:
        features[f'{prefix}_accel_lag1'] = float(mom.iloc[-1]) - float(mom.iloc[-2])
    if len(available_df) >= 4:
        features[f'{prefix}_accel_lag3'] = float(mom.iloc[-3]) - float(mom.iloc[-4])
    if len(available_df) >= 7:
        features[f'{prefix}_accel_lag6'] = float(mom.iloc[-6]) - float(mom.iloc[-7])
...
def batch_lagged_target_features(
    ...
    # Acceleration = diff(MoM); lagged so always PIT-safe (uses lag-1 minimum)
    # accel_lag1[t] = MoM[t-1] - MoM[t-2]  (observable before t's release)
    # accel_lag3[t] = MoM[t-3] - MoM[t-4]
    # accel_lag6[t] = MoM[t-6] - MoM[t-7]
    accel = mom.diff(1)                           # accel[t] = MoM[t] - MoM[t-1]
