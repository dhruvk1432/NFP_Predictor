...
def _run_fred_connectivity_preflight(as_of_str: str) -> None:
    """
    Fail fast when FRED is unreachable instead of iterating all series.

    This probe verifies:
    1) DNS resolution for api.stlouisfed.org
    2) A minimal API read path (PAYEMS as-of request)
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"FRED preflight failed: cannot resolve host '{host}'. "
            f"Underlying error: {e}"
        ) from e

    fred = Fred(api_key=FRED_API_KEY)
    try:
        probe = fred.get_series_as_of_date("PAYEMS", as_of_date=as_of_str)
    except Exception as e:
        raise RuntimeError(
            f"FRED preflight failed: probe request for PAYEMS as-of {as_of_str} failed. "
            f"Underlying error: {e}"
        ) from e

    if probe is None or probe.empty:
        raise RuntimeError(
            f"FRED preflight failed: PAYEMS probe returned no rows for as-of {as_of_str}."
        )
...
def _validate_single_target_file(path: Path, start_date: str, end_date: str) -> List[str]:
    """
    Validate one required target file for schema and requested time-window coverage.
    """
    errors: List[str] = []
    if not path.exists():
    ...
    try:
        ...
        return [f"{path}: unable to read parquet ({e})"]

    required_cols = {"ds", "y", "release_date"}
    missing_cols = sorted(required_cols - set(df.columns))
    if missing_cols:
        errors.append(f"{path}: missing required columns {missing_cols}")
        return errors
    ...
    if ds.isna().any():
        errors.append(f"{path}: contains unparsable ds values")
    if release_dates.isna().any():
        errors.append(f"{path}: contains missing/unparsable release_date values")
    if ds.duplicated().any():
        errors.append(f"{path}: contains duplicate ds rows")
    if y_numeric.notna().sum() == 0:
        errors.append(f"{path}: all y values are NaN")
    ...
    if not ds.dropna().empty:
        ...
        window_start = pd.to_datetime(start_date).to_period("M").to_timestamp()
        window_end = pd.to_datetime(end_date).to_period("M").to_timestamp()
        if min_ds > window_start or max_ds < window_end:
            errors.append(
                f"{path}: ds range {min_ds.strftime('%Y-%m')}..{max_ds.strftime('%Y-%m')} "
                f"does not cover requested {window_start.strftime('%Y-%m')}..{window_end.strftime('%Y-%m')}"
            )

    return errors


def validate_required_nfp_targets(start_date: str, end_date: str, raise_on_error: bool = True) -> bool:
    """
    Validate core target files that all downstream loaders depend on.

    Returns:
        True if all required files are valid; False when raise_on_error=False and
        validation fails.
    """
    ...
    for path in required_target_files:
        all_errors.extend(_validate_single_target_file(path, start_date, end_date))

    if all_errors:
        message = "NFP target validation failed:\n  - " + "\n  - ".join(all_errors)
        if raise_on_error:
...
def _validate_single_revised_target_file(path: Path, start_date: str, end_date: str) -> List[str]:
    """
    Validate one revised target file generated by build_revised_target().

    Revised targets intentionally start one month after START_DATE because y_mom
    requires M and M-1 levels.
    """
    errors: List[str] = []
    if not path.exists():
        return [f"{path}: file does not exist"]
    ...
    required_cols = {"ds", "y", "y_mom", "release_date"}
    missing_cols = sorted(required_cols - set(df.columns))
    if missing_cols:
        errors.append(f"{path}: missing required columns {missing_cols}")
        return errors
    ...
    y_mom = pd.to_numeric(df["y_mom"], errors="coerce")
    release_dates = pd.to_datetime(df["release_date"], errors="coerce")
    ...
    if y.notna().sum() == 0:
        errors.append(f"{path}: all y values are NaN")
    if y_mom.notna().sum() == 0:
        errors.append(f"{path}: all y_mom values are NaN")

    ds_non_na = ds.dropna()
    if not ds_non_na.empty:
        min_ds = ds_non_na.min().to_period("M").to_timestamp()
        max_ds = ds_non_na.max().to_period("M").to_timestamp()
        expected_start = (pd.to_datetime(start_date).to_period("M").to_timestamp() + pd.DateOffset(months=1))
        window_end = pd.to_datetime(end_date).to_period("M").to_timestamp()
        if min_ds > expected_start or max_ds < window_end:
            errors.append(
                f"{path}: ds range {min_ds.strftime('%Y-%m')}..{max_ds.strftime('%Y-%m')} "
                f"does not cover revised window {expected_start.strftime('%Y-%m')}..{window_end.strftime('%Y-%m')}"
            )

    return errors


def validate_required_revised_targets(start_date: str, end_date: str, raise_on_error: bool = True) -> bool:
    """
    Validate revised target files consumed by revised train/master branches.
    """
    required_files = [
        NFP_target_DIR / "y_nsa_revised.parquet",
        NFP_target_DIR / "y_sa_revised.parquet",
    ]

    all_errors: List[str] = []
    for path in required_files:
        all_errors.extend(_validate_single_revised_target_file(path, start_date, end_date))

    if all_errors:
        message = "Revised target validation failed:\n  - " + "\n  - ".join(all_errors)
        if raise_on_error:
            raise RuntimeError(message)
        logger.error(message)
        return False
...
def build_revised_target_parquets() -> tuple[Path, Path]:
    """
    Build revised target cache files in data/NFP_target via canonical implementation.
    """
    from Prepare_Data.build_revised_targets import build_and_save

    logger.info("Building revised target parquets (NSA + SA)")
    nsa_path = build_and_save("nsa")
    sa_path = build_and_save("sa")
    return nsa_path, sa_path
...
def _is_fatal_fred_error(exc: Exception) -> bool:
    """
    Fatal errors should fail immediately (no retry).
    """
...
def _is_retryable_fred_error(exc: Exception) -> bool:
    """
    Retryable errors are treated as transient transport/server issues.
    """
    text = _error_text(exc)
    retryable_patterns = (
        "rate limit",
...
def _fetch_with_retry(fred, fid, uid, as_of_str, max_retries=3, base_delay=30):
    """Fetch with classification-aware retry policy."""
    last_error = None
    for attempt in range(max_retries):
        try:
            ...
            last_error = e
            if _is_fatal_fred_error(e):
                raise RuntimeError(f"Fatal FRED error for {uid}: {e}") from e

            if _is_retryable_fred_error(e) and attempt < max_retries - 1:
                # Exponential backoff + small jitter to desynchronize bursts.
                wait_time = base_delay * (2 ** attempt) + random.uniform(0.0, 2.0)
                logger.warning(
                    f"Retrying {uid} in {wait_time:.1f}s "
...
def download_master_audit(
    ...
    logger.info("Starting FRED snapshots download")
    ...
    logger.info("Running FRED connectivity preflight")
    _run_fred_connectivity_preflight(as_of_str)
    logger.info("FRED connectivity preflight passed")
    ...
    loaded_uids = set(new_data["unique_id"].astype(str).unique())
    missing_required_uids = [uid for uid in TOTAL_UIDS if uid not in loaded_uids]
    if missing_required_uids:
        raise RuntimeError(
            "Download completed but required top-level FRED targets are missing: "
            f"{missing_required_uids}"
        )

    missing_required_rows = [uid for uid in TOTAL_UIDS if new_data[new_data["unique_id"] == uid].empty]
    if missing_required_rows:
        raise RuntimeError(
            "Download completed but required top-level FRED targets have zero rows: "
...
def build_all_snapshots(start_date=START_DATE, end_date=END_DATE, refresh_existing=False):
    ...
    if not refresh_existing:
        ...
        targets_valid = False
        if targets_exist:
            targets_valid = validate_required_nfp_targets(
                start_date=start_date,
                end_date=end_date,
                raise_on_error=False,
            )
        all_targets_exist = targets_exist and targets_valid

        if targets_exist and not targets_valid:
            logger.info("Existing NFP target files failed validation; rebuilding.")
    ...
    required_audit_uids = set(TOTAL_UIDS)
    available_audit_uids = set(audit["unique_id"].astype(str).unique()) if "unique_id" in audit.columns else set()
    missing_audit_uids = sorted(required_audit_uids - available_audit_uids)
    if missing_audit_uids:
        raise RuntimeError(
            "Audit data missing required top-level series: "
            f"{missing_audit_uids}"
        )
    ...
    validate_required_nfp_targets(start_date=start_date, end_date=end_date, raise_on_error=True)

    s, e = build_monthly_snapshots_from_audit(audit, start_date, end_date, refresh_existing, None)

    # Build revised target cache files used by revised master/train branches.
    build_revised_target_parquets()
    validate_required_revised_targets(start_date=start_date, end_date=end_date, raise_on_error=True)
...
def verify_prepared_snapshot():
    ...
    prepared_source = ""
    candidate_dirs = [
        ("NSA", PREPARED_FRED_NSA_DIR),
        ("SA", PREPARED_FRED_SA_DIR),
        ("LEGACY", PREPARED_FRED_DIR),
    ]
    last_attempts: List[Path] = []
    ...
    for _ in range(12):
        ...
        attempted = [
            base_dir / decade_str / year_str / f"{test_date.strftime('%Y-%m')}.parquet"
            for _, base_dir in candidate_dirs
        ]
        last_attempts = attempted

        for source_label, candidate in zip([s for s, _ in candidate_dirs], attempted):
            if candidate.exists():
                prepared_path = candidate
                prepared_source = source_label
                break
    ...
    if prepared_path is None:
        attempts_str = ", ".join(str(p) for p in last_attempts)
        logger.error(f"Verification failed: Could not find prepared snapshot in any expected location. Last attempts: {attempts_str}")
        return

    logger.info(f"Using prepared snapshot source: {prepared_source} ({prepared_path})")
    ...
    # NSA prepared snapshots use `total_nsa` naming; SA/legacy use `total`.
    test_series = 'total_nsa' if prepared_source == "NSA" else 'total'
    expected_patterns = [
