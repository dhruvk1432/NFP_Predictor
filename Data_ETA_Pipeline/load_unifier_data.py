...
# Reuters/LSEG NFP consensus poll (economists' forecasts)
# - Mean: average across all polled economists
# - Median: robust central tendency (resistant to outlier forecasts)
# - Good: subset of higher-accuracy ("good") economists per LSEG ranking
# - High / Low: max & min forecasts → dispersion / forecast uncertainty signal
CONSENSUS_POLL_NAME = "lseg_us_reuters_polls"
CONSENSUS_SERIES = {
    "NFP_Consensus_Mean":   "US&NFAREO",
    "NFP_Consensus_Median": "US&NFARMO",
    "NFP_Consensus_Good":   "US&NFARFO",
    "NFP_Consensus_High":   "US&NFARHO",
    "NFP_Consensus_Low":    "US&NFARLO",
}
...
def _fetch_consensus_series():
    """
    Fetch all NFP consensus poll series (Mean, Median, Good, High, Low) from Unifier.

    Returns a long-format DataFrame with the same schema as other Unifier series:
    ...
    Each series flows through the standard snapshot pipeline (pct_change,
    feature engineering) downstream — no special-case transformations needed
    here beyond the per-series PIT alignment.
    """
    frames = []
    for series_name, key in CONSENSUS_SERIES.items():
        try:
            df = unifier.get_dataframe(
                name=CONSENSUS_POLL_NAME, key=key, asof_date=None
            )
        except Exception as e:
            logger.error(f"Failed to fetch consensus poll {series_name} ({key}): {e}")
            continue

        if df.empty:
            logger.warning(f"Consensus poll {series_name} ({key}) returned no data")
            continue

        if "timestamp" not in df.columns or "first_release_value" not in df.columns:
            logger.error(
                f"Consensus poll {series_name} ({key}) missing required columns. "
        ...
        if out.empty:
            logger.warning(f"Consensus poll {series_name} ({key}) had no valid rows after cleaning")
            continue

        # Normalise to 1st-of-month (timestamp is typically mid-month)
        ...
        out = out[["date", "release_date", "value", "series_name", "series_code"]]

        logger.info(f"Fetched {len(out)} {series_name} observations")
        frames.append(out)
...
def fetch_unifier_snapshots(start_date=START_DATE, end_date=END_DATE):
    ...
    for name, code in UNIFIER_SERIES.items():
        try:
            ...
            if missing_required:
                logger.error(f"Schema error for {name}: missing {missing_required}")
                continue

            missing_optional = [c for c in optional_cols if c not in df.columns]
            if missing_optional:
                logger.warning(f"{name}: Missing {missing_optional}, using defaults")
    ...
    for name, df in all_series_data.items():
        try:
            ...
            logger.warning(f"NFP offset calculation failed for {name}: {e}")
