...
def generate_comparison_scorecard(
    ...
    for model_id, info in all_results.items():
        ...
        row = {
            ...
            '50%_Coverage': bt_valid['in_50_interval'].mean() * 100,
            '80%_Coverage': bt_valid['in_80_interval'].mean() * 100,
            '95%_Coverage': bt_valid['in_95_interval'].mean() * 100,
...
def _save_styled_html(scorecard: pd.DataFrame, path: Path) -> None:
    ...
    error_cols = ['RMSE', 'MAE', 'Median_AE', 'Max_AE']
    ...
    coverage_cols = ['50%_Coverage', '80%_Coverage', '95%_Coverage']
